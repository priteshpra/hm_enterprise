...
class Country extends Admin_Controller {
    ...
    function __construct() {
        parent::__construct();
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(" . $this->UserRoleID . ",23)");
        $tmp->next_result();
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/country/list', $array);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->country_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/country/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('MobileCode', 'MobileCode', 'trim|required');
            if ($this->form_validation->run() == TRUE) {
                $res = $this->country_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url('admin/masters/country'));
                }else{
                    ...
                    redirect(site_url('admin/masters/country/add'));
                }
            } else {
        ...
        $res['page_level_js'] = $this->load->view('admin/masters/country/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->country_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->country_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url('admin/masters/country'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/masters/country/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/masters/country/edit/' . $ID));
    ...
    public function export_to_excel() {
        ...
        $array['data'] = $this->country_model->ListData(-1, 1);

...
class Cms extends Admin_Controller {
    ...
    function __construct() {
        parent::__construct();
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(" . $this->UserRoleID . ",4)");
        $tmp->next_result();
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->cms_model->ListData($PageSize, $CurrentPage);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (@$res->ID) {
                    redirect(site_url('admin/masters/cms'));
                }else{
                    ...
                    redirect(site_url('admin/masters/cms/add'));
                }
            } else {
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->cms_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (@$res->ID) {
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/masters/cms/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/masters/cms/edit/' . $ID));
        ...
        $data['loading_button'] = getLoadingButton();
        $data['Page'] = getPageCombobox($data['data']->PageID);
        $this->load->view('admin/masters/cms/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/cms/add_edit_js', NULL, TRUE);
        $this->load->view('admin/includes/footer', $res);
        unset($data,$res);

...
class Subcategory extends Admin_Controller {
    ...
    function __construct() {
        ...
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(".$this->UserRoleID . ",27)");
        $tmp->next_result();
    ...
    public function index() {
        $array = $data = array();
        $array['Category'] = getCategoryComboBox();
        $this->load->view('admin/includes/header');
        $array['view_modal_popup'] = $this->load->view('admin/includes/view_modal_popup', NULL, TRUE);
        $this->load->view('admin/masters/subcategory/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/subcategory/list_js', NULL, TRUE);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->subcategory_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/subcategory/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('SubCategoryName', 'SubCategoryName', 'trim|required');
            if ($this->form_validation->run() == TRUE) {
                $res = $this->subcategory_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/subcategory'));
                }else{
                    ...
                    redirect(site_url( 'admin/masters/subcategory/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/subcategory/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/subcategory/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->subcategory_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->subcategory_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/subcategory'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url( 'admin/masters/subcategory/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url( 'admin/masters/subcategory/edit/' . $ID));
        ...
        $data['Category'] = getCategoryComboBox(@$data['data']->CategoryID);
        $data['loading_button'] = getLoadingButton();
        $this->load->view('admin/masters/subcategory/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/subcategory/add_edit_js', NULL, TRUE);
        $this->load->view('admin/includes/footer', $res);
    ...
    public function Combobox(){
        if($this->input->get()){            
            $json = array();
            $_POST['SubCategoryName'] = ($this->input->get('q')=="")?'':$this->input->get('q');
            $data = $this->subcategory_model->ListData(-1,1);
            foreach ($data as $key => $value) {
                if(@$value->Message)
                    break;
                $json[] = ['id'=>$value->SubCategoryID, 'text'=>$value->SubCategoryName];
            }
    ...
    public function export_to_excel() {
        ...
        $array['data'] = $this->subcategory_model->ListData(-1, 1);

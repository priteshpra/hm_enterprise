...
class Reason extends Admin_Controller {
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/reason/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/reason/list_js', NULL, TRUE);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->reason_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/reason/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $res = $this->reason_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/reason'));
                }else{
                    ...
                    redirect(site_url( 'admin/masters/reason/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/reason/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/reason/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->reason_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->reason_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/reason'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url( 'admin/masters/reason/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url( 'admin/masters/reason/edit/' . $ID));
    ...
    public function export_to_excel() {
        ...
        $array['data'] = $this->reason_model->ListData(-1, 1);
        ...
        ob_end_clean();
        $filename = 'Reason.xls'; //save our workbook as this file name

...
class Service extends Admin_Controller {
    ...
    function __construct() {
        ...
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(".$this->UserRoleID . ",28)");
        $tmp->next_result();
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/service/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/service/list_js', NULL, TRUE);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->service_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/service/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $res = $this->service_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/service'));
                }else{
                    ...
                    redirect(site_url( 'admin/masters/service/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/service/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/service/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->service_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->service_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/service'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url( 'admin/masters/service/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url( 'admin/masters/service/edit/' . $ID));
    ...
    public function Combobox(){
        if($this->input->get()){            
            $json = array();
            $_POST['Service'] = ($this->input->get('q')=="")?'':$this->input->get('q');
            $data = $this->service_model->ListData(-1,1);
            foreach ($data as $key => $value) {
                if(@$value->Message)
                    break;
                $json[] = ['id'=>$value->ServiceID, 'text'=>$value->Service];
            }
    ...
    public function export_to_excel() {
        ...
        $array['data'] = $this->service_model->ListData(-1, 1);
        ...
        ob_end_clean();
        $filename = 'Service.xls'; //save our workbook as this file name

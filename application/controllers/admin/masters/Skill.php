...
class Skill extends Admin_Controller {
    ...
    function __construct() {
        parent::__construct();
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(" . $this->UserRoleID . ",19)");
        $tmp->next_result();
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/skill/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/skill/list_js', NULL, TRUE);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->skill_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/skill/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('SkillName', 'SkillName', 'trim|required');
            if ($this->form_validation->run() == TRUE) {
                $res = $this->skill_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/skill'));
                }else{
                    ...
                    redirect(site_url( 'admin/masters/skill/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/skill/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/skill/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->skill_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->skill_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/skill'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url( 'admin/masters/skill/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url( 'admin/masters/skill/edit/' . $ID));
    ...
    public function export_to_excel() {
        ...
        $array['data'] = $this->skill_model->ListData(-1, 1);
        ...
        ob_end_clean();
        $filename = 'Skill.xls'; //save our workbook as this file name

...
class Emailtemplate extends Admin_Controller {
    ...
    function __construct() {
        parent::__construct();
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(" . $this->UserRoleID . ",7)");
        $tmp->next_result();
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/emailtemplate/list', $array);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->emailtemplate_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/emailtemplate/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (@$res->ID) {
                    redirect(site_url('admin/masters/emailtemplate'));
                }else{
                    ...
                    redirect(site_url('admin/masters/emailtemplate/add'));
                }
            } else {
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->emailtemplate_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (@$res->ID) {
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/masters/emailtemplate/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/masters/emailtemplate/edit/' . $ID));

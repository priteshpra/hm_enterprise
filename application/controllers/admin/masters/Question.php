...
class Question extends Admin_Controller {
    ...
    function __construct() {
        parent::__construct();
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(" . $this->UserRoleID . ",20)");
        $tmp->next_result();
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/question/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/question/list_js', NULL, TRUE);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->question_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/question/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $data['Questionoption'] = implode(',',$data['Questionoption']);
                $res = $this->question_model->Insert($data);
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/question'));
                } else {
                    ...
                    redirect(site_url( 'admin/masters/question/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/question/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/question/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $data['Questionoption'] = implode(',',$data['Questionoption']);
                $res = $this->question_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/question'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url( 'admin/masters/question/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url( 'admin/masters/question/edit/' . $ID));
    ...
    public function export_to_excel() {
        ...
        $array['data'] = $this->question_model->ListData(-1, 1);
        ...
        ob_end_clean();
        $filename = 'Question.xls'; //save our workbook as this file name

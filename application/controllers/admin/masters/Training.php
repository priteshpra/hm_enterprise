...
class Training extends Admin_Controller {
    ...
    function __construct() {
        ...
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(".$this->UserRoleID . ",31)");
        $tmp->next_result();
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/training/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/training/list_js', NULL, TRUE);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->training_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/training/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('TrainingData', 'TrainingData', 'trim|required');
            if ($this->form_validation->run() == TRUE) {
                $res = $this->training_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/training'));
                }else{
                    ...
                    redirect(site_url( 'admin/masters/training/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/training/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/training/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->training_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->training_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/training'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url( 'admin/masters/training/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url( 'admin/masters/training/edit/' . $ID));
    ...
    public function Combobox(){
        if($this->input->get()){            
            ...
            $data = $this->training_model->ListData(-1,1);
    ...
    public function export_to_excel() {
        ...
        $array['data'] = $this->training_model->ListData(-1, 1);

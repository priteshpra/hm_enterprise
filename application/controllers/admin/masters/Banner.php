...
class Banner extends Admin_Controller
{
    ...
    {
        parent::__construct();
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(" . $this->UserRoleID . ",17)");
        $tmp->next_result();
    ...
    public function index()
    {
        ...
        $this->load->view('admin/masters/banner/list', $array);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1)
    {
        ...
        $res['data_array'] = $this->banner_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/banner/ajax_listing', $res, TRUE);
    ...
    public function add()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $url = site_url("admin/master/banner/add");
                $config = array(
                ...
                if (@$res->ID) {
                    redirect(site_url('admin/masters/banner'));
                } else {
                    ...
                    redirect(site_url('admin/masters/banner/add'));
                }
            } else {
    ...
    {
        $data = $res = array();
        $data['data'] = $this->banner_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $data = $this->input->post();
                $url = site_url("admin/master/banner/edit");
                $config = array(
                    "max_width" => BANNER_MAX_WIDTH,
                    "max_height" => BANNER_MAX_HEIGHT,
                    'max_size' => BANNER_MAX_SIZE,
                ...
                if (@$res->ID) {
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/masters/banner/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/masters/banner/edit/' . $ID));

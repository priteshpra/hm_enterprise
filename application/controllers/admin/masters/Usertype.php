...
class Usertype extends Admin_Controller {
    ...
    function __construct() {
        ...
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(".$this->UserRoleID . ",30)");
        $tmp->next_result();
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/usertype/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/usertype/list_js', NULL, TRUE);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->usertype_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/usertype/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('UsertypeData', 'UsertypeData', 'trim|required');
            if ($this->form_validation->run() == TRUE) {
                $res = $this->usertype_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/usertype'));
                }else{
                    ...
                    redirect(site_url( 'admin/masters/usertype/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/usertype/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/usertype/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->usertype_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->usertype_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/usertype'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url( 'admin/masters/usertype/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url( 'admin/masters/usertype/edit/' . $ID));
    ...
    public function Combobox(){
        if($this->input->get()){            
            ...
            if($this->input->get('IsMaterial') == "0") {
                $data = $this->usertype_model->ListUserData(-1,1);
            } else if($this->input->get('IsMaterial') == "1") {
                $data = $this->usertype_model->ListMaterialData(-1,1);
            } /* else {
                $data = $this->usertype_model->ListData(-1,1);
            } */
            foreach ($data as $key => $value) {
                if(@$value->Message)
                    break;
                $json[] = ['id'=>$value->UsertypeID, 'text'=>$value->Usertype, 'Rate'=>$value->Rate, 'HSNNo'=>$value->HSNNo];
            }
    ...
    public function export_to_excel() {
        ...
        $array['data'] = $this->usertype_model->ListData(-1, 1);

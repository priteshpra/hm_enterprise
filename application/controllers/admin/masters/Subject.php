...
class Subject extends Admin_Controller {
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/subject/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/subject/list_js', NULL, TRUE);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->subject_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/subject/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $res = $this->subject_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/subject'));
                }else{
                    ...
                    redirect(site_url( 'admin/masters/subject/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/subject/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/subject/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->subject_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->subject_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/subject'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url( 'admin/masters/subject/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url( 'admin/masters/subject/edit/' . $ID));
    ...
    public function Combobox(){
        if($this->input->get()){            
            $json = array();
            $_POST['SubjectData'] = ($this->input->get('q')=="")?'':$this->input->get('q');
            $data = $this->subject_model->ListData(-1,1);
            foreach ($data as $key => $value) {
                if(@$value->Message)
                    break;
                $json[] = ['id'=>$value->SubjectID, 'text'=>$value->Subject];
            }
    ...
    public function export_to_excel() {
        ...
        $array['data'] = $this->subject_model->ListData(-1, 1);
        ...
        ob_end_clean();
        $filename = 'Subject.xls'; //save our workbook as this file name

...
class State extends Admin_Controller {
    ...
    function __construct() {
        parent::__construct();
        $tmp =  $this->db->query("CALL usp_GetRoleMappingByID(" . $this->UserRoleID . ",24)");
        $tmp->next_result();
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/state/list', $array);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->state_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['TotalRecords'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/state/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (@$res->ID) {
                    redirect(site_url('admin/masters/state'));
                }else{
                    ...
                    redirect(site_url('admin/masters/state/add'));
                }
            } else {
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->state_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (@$res->ID) {
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/masters/state/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/masters/state/edit/' . $ID));

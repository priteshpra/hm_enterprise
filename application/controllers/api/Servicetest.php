...
class Service extends MY_Controller
{
    ...
    function test($data)
    {
        /* $device = $this->getDeviceData(1);
        $employee = $this->getEmployeeData(1);
        if(@$device != "") {
            $notif_text = "this is testing message";
        ...
        if ($device != "") {
            ...
            if (isset($_result_quotation[0]->QuotationID)) {
                ...
                if (isset($_result_sites[0]->SitesID)) {
                    echo $employee . " has added a new Quotation of the " . $_result_sites[0]->SiteName . ", Quotation No: " . $_result_quotation[0]->EstimateNo . " on " . $_result_quotation[0]->EstimateDate . "";
                    die;
                    $pushNotificationArr = array(
                        'device_id' => $device,
                        'message' => $employee . " has added a new Quotation of the , Quotation No: 998 on " . date('d-m-Y') . "",
        ...
        /* $res = $this->master_model->getQueryResult("call usp_A_GetProductByChallanItemID('62')");
        if(!isset($res[0]->Message)) {
            ...
            for($i=0; $i<$service; $i++) {
                ...
                $data->UserID = "1";
                $IP = GetIP();
                $res = $this->master_model->getQueryResult("call usp_A_AddproductService('".
                ...
                //echo $service_date = date('Y-m-d', strtotime($service_date, '+100 Days'));
            }
            
            $res = $this->master_model->getQueryResult("call usp_A_AddproductWarranty('".
                $data->UserID . "','1','Android','" .$IP . "','" .
                $ChallanItemID . "','" .
                $InstallationID . "','" .
                $date . "','" .
        ...
        $configdata = $this->config_model->getConfig();
        pr($configdata);
        die;
        $timeslot = array();
        for($i=$start; $i<$end; $i++) {
    ...
    public function PrintReceipt($ID)
    {
        ...
        $html .=     '<tr>
                            <td colspan="3" style="border-bottom: 0 solid white;"></td>
        ...
        $html .=            '<tr>
                        ...
                        <tr>
                            <td colspan="3" style="border-bottom: 1 solid white;"></td>
                            <td colspan="2"><b>Total</b></td>
                            <td style="text-align: right"><b>Rs. ' . ((float)$Quotation['0']->Total) . '</b></td>
                        </tr>
                        <tr>
                            <td colspan="6" style="text-transform: uppercase"><b>Total In Words: </b>' . numberTowords((int)((float)$Quotation['0']->Total)) . '</td>
                        </tr>
                    </table><br>
                    <div>
                       <b>Notes</b><br>

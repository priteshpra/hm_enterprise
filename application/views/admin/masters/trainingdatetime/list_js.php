...
$(document).on('click','.export-excel',function(){
    ...
    function common_ajax(PageSize,CurrentPage){
        $.ajax({
            type: "post",
            datatype:"JSON",
            url: base_url + "admin/masters/trainingdatetime/ajax_listing/" + PageSize + "/" + CurrentPage,
            data: {
    ...
    //---------pagiing and search----------//     
    $(document).ready(function () {
        $("#model_title").html("<?php echo label('msg_lbl_training');?>");
   ...
   $(".TableBody").on('click', '.status_change', function(){
        ...
        $.ajax({
            type:"post",
            url: base_url + "admin/masters/trainingdatetime/changeStatus",
            data:{
                Table :"TrainingDateTime",
                ID : ID,
                Status : Status,
            },success:function(data){
                var obj = JSON.parse(data);
    ...
    $(".TableBody").on('click', '.info', function(){
        ...
        var ID = $(this).attr('data-id');
        var Table = "TrainingDateTime";
        $.ajax({
            type: "post",
            url: base_url + "admin/masters/trainingdatetime/getRecordInfo",
